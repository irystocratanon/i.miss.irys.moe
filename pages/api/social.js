        ...
        await parseString(await tweets.text(), {trim: true}, async function(err, res) {
            ...
            await items.forEach((item,i) => {
                item.description = item.description.map(d => {
                    return d.replace(/piped.kavin.rocks\//g, 'www.youtube.com/watch?v=').replace(/youtu.be\//g, 'www.youtube.com/watch?v=').replace(endpointRegExp, 'href="https://twitter.com/').replace(/nitter.*\/(?<status>\w+)\/status/g, function(match, p1, p2, p3, offset, string) {return `twitter.com/${p1}/status`}).replace(/http:\/\//g, 'https://').replace(/#m\<\/a\>/g, '</a>').replaceAll("<img src=\"", "<img loading=\"lazy\" src=\"")
                })
                const isRetweet = rt => {
...
export async function getSocials(full = false) {
    ...
    if (full) {
        const spaceSearchTerms = ['?f=tweets&q=%23IRySpace&since=&until=&near=', '?f=tweets&q=twitter.com%2Fi%2Fspaces%2F&since=&until=&near=']
        for (const term of spaceSearchTerms) {
            for (let endpoint of nitterEndpoints) {
                let tweets
                try {
                    tweets = await fetchTweets(endpoint.replace(/https?:\/\//, ''), `${endpoint}/irys_en/search/rss${term}`)
                } catch (err) { console.error(err); }
                if (tweets.length === 0 || !tweets) { continue; }
                tweets.forEach(tweet => { if (socials.findIndex(e => { return e.id === tweet.id }) === -1) { socials.push(tweet); } })
                t1 = performance.now()
